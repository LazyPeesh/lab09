...
router.get('/list', async (req, res) => {
    var movies_list = await MovieModel.find({});
    res.render('movie/list', { movies: movies_list });
});
...
router.post('/edit/:id', async (req, res) => {
    // var movie = res.body;
    // await MovieModel.findByIdAndUpdate(res.params.id, movie)
    //     .then(() => { console.log("Add new movie succeed !") });
    // res.redirect('/movie');

    var data = req.body;
    var id = req.params.id;
    var movie = await MovieModel.findByIdAndUpdate(id, data);
    // console.log(movie);
    res.redirect('/movie');
});
...
// sort function
router.get('/list/asc', async (req, res) => {
    var movies = await MovieModel.find().sort({ title: 1 });
    res.render('movie/list', { movies: movies });
});

router.get('/list/desc', async (req, res) => {
    var movies = await MovieModel.find().sort({ title: -1 });
    res.render('movie/list', { movies: movies });
});
